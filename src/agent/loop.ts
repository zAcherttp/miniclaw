...
import { MessageBus, OutboundMessage } from '../bus/queue';
  ...
  private async processInbound() {
    while (this.running) {
      try {
        ...
        console.log('[AgentLoop] Received from ' + msg.channel + ' (' + msg.chat_id + '): ' + msg.content);
  ...
  private async processOutbound() {
    while (this.running) {
      try {
        ...
        console.log('[AgentLoop] Sending to ' + msg.channel + ' (' + msg.chat_id + '): ' + msg.content);
      } catch (e) {
        console.error('Error processing outbound message:', e);
      }
    }
  }
}
