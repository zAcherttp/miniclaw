	...
	return new DynamicStructuredTool({
		...
		schema: z.object({
			query: z
				.string()
				.describe(
					"Case-insensitive keyword to search for in skill names, descriptions, or categories.",
				),
		}),
		func: async ({ query }) => {
			try {
				...
				const matches = loadedSkills.filter((s) => {
					return (
				...
				if (matches.length === 0) {
					logger.info(`[Skills] No skills matched query "${query}"`);
					return `No skills matched your query "${query}". Try different keywords.`;
				}
