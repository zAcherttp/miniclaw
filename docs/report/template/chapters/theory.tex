...
Các mô hình ngôn ngữ lớn (Large Language Model - LLM) phát triển từ các kiến trúc mạng hồi quy (RNN) và LSTM truyền thống vốn bị giới hạn ở khả năng tính toán tuần tự và duy trì phụ thuộc tầm xa. Kiến trúc Transformer với cơ chế tự chú ý (self-attention) cho phép tính toán đồng thời mối liên hệ giữa mọi cặp phần tử trong chuỗi và khai thác triệt để khả năng song song hóa của phần cứng hiện đại. Trên nền tảng đó, các họ mô hình tự hồi quy (autoregressive) như GPT, LLaMA hay Qwen đã thể hiện khả năng mô phỏng ngôn ngữ tự nhiên và suy luận ở mức độ đáng kể.
...
Bộ nhớ dài hạn lưu trữ các sự kiện, thông tin cấu hình và thói quen mang tính bền vững của người dùng qua các phiên làm việc. Hệ thống không thể đưa toàn bộ bộ nhớ dài hạn vào prompt vì giới hạn token và chi phí tính toán. Thay vào đó, hệ thống phân tách thông tin dài hạn thành các phân đoạn độc lập và lưu vào kho lưu trữ cục bộ có gắn vector embedding để phục vụ truy xuất ngữ nghĩa \cite{memverge2026memmachine}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node (vector) [database, right=1.8cm of embed] {Kho vector\\cục bộ};
...
MiniClaw hiện thực hóa kiến trúc bộ nhớ phân tầng này thông qua việc sử dụng tệp lưu vết checkpoint cục bộ cho bộ nhớ ngắn hạn. Đồng thời, hệ thống triển khai mô hình nhúng kết hợp tìm kiếm tương đồng Cosine trên kho lưu trữ cục bộ, có fallback sang tìm kiếm từ khóa khi embedding không khả dụng, để truy xuất thông tin dài hạn phù hợp với ngữ cảnh hiện tại.
...
Thư viện LangGraph cung cấp mô hình trừu tượng hóa vòng lặp tác nhân dưới dạng đồ thị trạng thái (State Graph) \cite{langgraphDocs}. Trong kiến trúc này, các nút (Nodes) biểu diễn các bước xử lý dữ liệu và chia sẻ trạng thái chung của hệ thống (gồm lịch sử thông điệp và biến trạng thái nội bộ), còn các cạnh (Edges) chịu trách nhiệm điều phối dòng chảy:
\begin{itemize}
...
\begin{enumerate}
  \item \textbf{Chỉ đọc (Read-only):} Các tác vụ truy vấn dữ liệu không thay đổi trạng thái hệ thống, như đọc tệp tài liệu cấu hình, tìm kiếm ngữ nghĩa hoặc truy vấn kho vector cục bộ.
...
Để đảm bảo an toàn thực thi và giảm rủi ro tiêm nhiễm chỉ thị (prompt injection) dẫn đến kích hoạt mã độc, khung công cụ thiết lập cơ chế cô lập nghiêm ngặt. Hệ thống vận hành các tác vụ nguy cơ cao trong môi trường hộp cát (sandbox) hạn chế đặc quyền, đồng thời áp dụng các tầng lọc và xác thực tham số đầu vào trước khi chuyển đến tầng thực thi lõi.
...
Hệ thống sử dụng nền tảng backend Node.js nhờ cơ chế xử lý bất đồng bộ và hướng sự kiện (event-driven, non-blocking I/O) \cite{nodejsDocs}. Đặc tính này giúp tối ưu hóa việc truyền nhận dữ liệu và phản hồi theo thời gian thực của chatbot.

Ngôn ngữ TypeScript hỗ trợ kiểm tra kiểu tĩnh (static typing) nghiêm ngặt để quản lý mã nguồn chặt chẽ \cite{typescriptDocs}. Hệ thống phát hiện và ngăn chặn sớm các lỗi cấu trúc dữ liệu ngay trong giai đoạn biên dịch (compile-time), tránh lỗi vận hành (runtime) và tăng tính mô-đun hóa để dễ dàng bảo trì.

\subsection{grammY — Thư viện Telegram Bot}
Đồ án lựa chọn grammY làm thư viện chính để giao tiếp với Telegram Bot API \cite{grammyDocs}. grammY đạt hiệu năng xử lý bất đồng bộ cao trong môi trường Node.js và hỗ trợ tốt cho TypeScript. Với kiến trúc phân tầng mã trung gian (middleware) mạnh mẽ, grammY giúp hệ thống triển khai các bộ lọc thông điệp, quản lý phiên trò chuyện (session) và định dạng giao diện tương tác linh hoạt.

\subsection{LangGraph — Khung điều phối đồ thị trạng thái}
Để quản lý logic suy luận đa bước của tác nhân, đồ án áp dụng LangGraph làm khung điều phối (orchestration engine) theo mô hình đồ thị trạng thái (State Graph) \cite{langgraphDocs}. Giải pháp này giúp kiểm soát chi tiết tiến trình lập luận (reasoning), điều hướng linh hoạt giữa các trạng thái và xử lý các yêu cầu gọi công cụ song song.
...
\begin{itemize}
    \item \textbf{Ollama:} Hỗ trợ chạy cục bộ (self-hosted) các mô hình mã nguồn mở (như LLaMA hoặc Qwen) trên phần cứng giới hạn \cite{ollamaDocs}. Ollama giúp bảo vệ quyền riêng tư của dữ liệu và đảm bảo hệ thống hoạt động độc lập không phụ thuộc Internet.
